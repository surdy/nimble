...
An automatically managed file that stores runtime state. **Do not edit this file manually** — Nimble writes it whenever the active context changes.
...
The file is read on app startup to restore the previous context. External tools can read it to query the active context without interacting with the launcher window. See [External Context API](../guides/external-context.md) for usage examples.
